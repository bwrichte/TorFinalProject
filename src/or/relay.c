...
#define CELL_QUEUE_HIGHWATER_SIZE (get_options()->CircuitQueueHighwater)
/** Start reading from edge connections again when we get down to this many
 * cells. */
#define CELL_QUEUE_LOWWATER_SIZE (get_options()->CircuitQueueLowwater)
                           ...
                           cell_direction_t cell_direction)
{
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    cell->circ_id = circ->n_circ_id; /* switch it */
        or_conn = circ->n_conn; //relay the cell to the next or_connection
  ...
  ++stats_n_relay_cells_relayed; /* XXXX no longer quite accurate {cells}
                                  ...
                                  * the cells. */
//mashael: this is the next or_connection
...
//if there is no room in the circuit queue, return without packaging any cell
    if (get_options()->UseN23) {
        if (!CIRCUIT_IS_ORIGIN(circ)) {
            ...
            if ( queue->n > N2+N3 ) {  /* IG: This doesn't check the adaptive N3 value */
...
connection_edge_consider_sending_sendme(edge_connection_t *conn)
{
  ...
  if (get_options()->UseN23) {
    /* never send a sendme when N23 is on; it'll just be ignored anyway. */
    return;
  }
                                   ...
                                   crypt_path_t *layer_hint)
{
  ...
  if (get_options()->UseN23) {
    ...
    if (CELL_QUEUE_HIGHWATER_SIZE - cells_on_queue < circ->package_window)
                            ...
                            int block, streamid_t stream_id)
{
  ...
  int n = 0;
    if (circ->n_conn == orconn) { //if this connection I am writing to, is the circuits next connection
    circ->streams_blocked_on_n_conn = block;
    if (CIRCUIT_IS_ORIGIN(circ))
            edge = TO_ORIGIN_CIRCUIT(circ)->p_streams; //linked list of p_streams (streams between OP and application)
  } else {
...
//find the direction of the cell (the previous queue or the next queue)

  int cell_direction_p=0;
...
--For traffic going from the exit OR towards the OP, ORs perform the following depending on their position in the circuit:

1. Entry OR:
Resets the circuit's credit_balance_p because we do not expect a flow control (or credit) cell from the
OP. Also, it checks if it needs to send a flow_control (or credit) cell to the middle OR.
2. Middle OR:
 Makes the circuit inactive if the circuit's credit_balance_p is equal or less than 0. It also checks
...
static int
connection_or_consider_sending_flowcontrol_cell(int cell_direction_p, int nBuffer,
                                                circuit_t *circ,
                                                or_connection_t * orconn)
{
    ...
    /* IG: refactor this so that it ends up with a single call to
     * connection_or_send_flowcontrol()  [or maybe one for each
     * direction]. */

    if (cell_direction_p) { //cell headed IN (from server towards OP). (going previous previous previous)
        or_circuit_t *or_circ;
        previous_or = circ->n_conn;
        circ_id =circ->n_circ_id;
        or_circ = TO_OR_CIRCUIT(circ);
        or_circ->credit_balance_p--;
...
else { //cell headed OUT (going next next next)
        previous_or = TO_OR_CIRCUIT(circ)->p_conn;
