...
static void command_process_flowcontrol_cell(cell_t *cell, or_connection_t *conn);
...
#define PROCESS_CELL(tp, cl, cn) command_process_ ## tp ## _cell(cl, cn)
  ...
  switch (cell->command) {
    ...
    case CELL_FLOWCONTROL:
      /* IG: Keep stats of flow control cells? */
      PROCESS_CELL(flowcontrol, cell, conn);
      break;
    default:
...
command_process_flowcontrol_cell(cell_t *cell, or_connection_t *conn)
{
	...
	circ->cells_fwded_n -= cells_fwded_neighbor;
       and then just:
        credit_balance_n = N2 + N3 - circ->cells_fwded_n;
    ...
    /*Mashael: moved the function call to the above if-else*/
  //  make_circuit_active_on_conn(circ,conn);
	...
	/*Mashael: commented out the call and moved to the above if-else instead*/
      //  circuit_resume_edge_reading(circ,NULL);
    }
...
command_process_created_cell(cell_t *cell, or_connection_t *conn)
{
  ...
  circ = circuit_get_by_circid_orconn(cell->circ_id, conn);
    //printf("command process created cell %d \n", cell->circ_id);
  if (!circ) {
    log_info(LD_OR,
